...
public class PersistenciaTests extends PersistenciaFixture {
	...
	public void testCasoDePrueba1() {
		...
		DispositivoInteligente dispositivoInteligente = new DispositivoInteligente("Televisor",
				LocalDate.of(2017, 3, 28), sony, 0, 0);
		...
		Cliente unCliente = new Cliente("Fede", "Perez", TipoDocumento.DNI, "41919911", "23456379",
				"De la puerta para adentro", LocalDate.of(2018, 5, 20), dispositivosInteligentes,
				dispositivosEstandares, false, new Punto(-0.127512, 51.507222), null);
	...
	public void testCasoDePrueba3() {
		...
		DispositivoInteligente dispositivo = new DispositivoInteligente("Smartphone", LocalDate.now(), fabricante,
				150.0, 300.0);
		...
		Assert.assertEquals(Operador.MAYOR_IGUAL + " 25.0",
				condicionPersistida.getOperador() + " " + condicionPersistida.getValor());
	}
	...
	public void testCasoDePrueba5() {
		...
		DispositivoInteligente aireAcondicionado = new DispositivoInteligente("Aire acondicionado",
				LocalDate.of(2017, 4, 28), sony, 90.0, 360.0);
		...
		Cliente cliente = new Cliente("Jorge", "Perez", TipoDocumento.DNI, "1111", "4444", "Nazca 156",
				LocalDate.of(2017, 4, 28), inteligentes, dispositivosEstandar, true, new Punto(-0.127512, 51.507222),
				null);
		...
		manager.getTransaction().begin();
		aireAcondicionado = manager.find(DispositivoInteligente.class,
				cliente.getDispositivosInteligentes().get(0).getId());
		System.out.println("Promedio consumo dispositivo = " + aireAcondicionado.consumoTotalEnPeriodo(inicio, fin)
				/ aireAcondicionado.getRegistrosConsumo().size());
		manager.getTransaction().commit();
		...
		System.out.println("Promedio consumo transformador = " + transformador.consumoTotalEnPeriodo(inicio, fin)
				/ transformador.getClientes().get(0).dispositivosInteligentes().get(0).getRegistrosConsumo().size());
		manager.getTransaction().commit();
		...
		transformador.getClientes().get(0).getDispositivosInteligentes().get(0).getRegistrosConsumo().stream()
				.forEach(res -> res.setKwConsumidos(res.getKwConsumidos() * 10));
		manager.getTransaction().commit();

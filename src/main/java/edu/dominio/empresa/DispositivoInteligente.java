...
public class DispositivoInteligente extends Dispositivo {
	...
	public double consumoTotalEnPeriodo(LocalDate inicio, LocalDate fin) {
		...
		return registrosConsumo.stream().filter(registro -> registro.estaEntreFechas(inicio, fin))
				.mapToDouble(registro -> registro.kwConsumidos()).sum();
	}

	public double horasTotalesEnPeriodo(LocalDate inicio, LocalDate fin) {
		// se asume, y se van a guardar de manera ordenada los registros
		System.out.println("REGISTROS = "
				+ registrosConsumo.stream().filter(registro -> registro.estaEntreFechas(inicio, fin)).count());
		return registrosConsumo.stream().filter(registro -> registro.estaEntreFechas(inicio, fin))
				.mapToDouble(registro -> registro.horasEncendido()).sum();
	}

	// estos metodos se los delega al fabricante que se comunica con el dispositivo
	// fisico.

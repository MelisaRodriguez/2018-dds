...
public class Server {
	...
	static void setCrons() {
		try {
			...
			Trigger trigger = newTrigger().withSchedule(simpleSchedule().withIntervalInHours(24).repeatForever())
					.build();
			...
			Trigger trigger2 = newTrigger().withSchedule(simpleSchedule().withIntervalInHours(4).repeatForever())
					.build();
	...
	public static class DispositivosJob implements Job {
		...
		public void execute(JobExecutionContext context) throws JobExecutionException {
			List<Cliente> clientes = RepoClientes.getInstanceOfSingleton().getEntidades();
			List<DispositivoInteligente> dispositivos = clientes.stream()
					.flatMap(c -> c.getDispositivosInteligentes().stream()).collect(Collectors.toList());
			// dispositivos.stream().forEach(d -> d.setFabricante(new Fabricante("Sony",
			// FabricanteAdapter.SONY)));
